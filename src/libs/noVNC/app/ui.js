 ...
 * ------v------*/
	...
	connect(url, token) {
		...
		// 		UI.showStatus("Must set host", 'error');
		// 		return;
		// }
	...
	connectFinished(e) {
		...
		let msg = "Connected to " + UI.desktopName;;
	...
	disconnectFinished(e) {
		...
		if (!e.detail.clean) {
			...
			if (wasConnected) {
				...
				UI.showStatus("Failed to connect to server", 'error');
			}
			...
			UI.updateVisualState('disconnected');
			UI.showStatus("Disconnected", 'normal');
		}
	...
	securityFailed(e) {
		...
		if ('reason' in e.detail) {
			msg = "New connection has been rejected with reason: " +
				e.detail.reason;
		} else {
 ...
 * ------v------*/
	...
	credentials(e) {
		...
		UI.showStatus("Credentials are required", "warning");
	},
