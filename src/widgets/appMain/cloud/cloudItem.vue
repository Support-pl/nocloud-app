<template>
  <div
    class="cloud__item-wrapper"
    @click="(e) => cloudClick(instance, e)"
  >
    <div class="cloud__item">
      ...
      <div
        v-if="networking.length < 1 && instance.groupname === 'Self-Service VDS SSD HC'"
        class="item__status"
      >
        IP: {{ $t("ip.none") }}
      </div>
      <div
        v-else-if="networking.length < 2"
        class="item__status"
      >
...
export default {
  ...
  computed: {
    ...
    getModuleProductBtn () {
      ...
      const components = import.meta.glob('@/widgets/services/*/lilbtn.vue')
