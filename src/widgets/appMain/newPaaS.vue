<template>
  <div class="newCloud_wrapper">
    <div class="newCloud">
      ...
      <div
        v-if="itemSP && getPlans.length > 0"
        class="newCloud__calculate field result"
      >
        <div
        ...
        >
          ...
          >
            ...
            >
              ...
              <a
                href="#"
                @click.prevent="availableLogin('login')"
              >{{ $t("unregistered.login") }}</a>.
              <br>
              <a
                href="#"
                @click.prevent="availableLogin('copy')"
              >
                {{ $t("Copy link") }}
                <a-icon type="copy" />
              </a>
...
import addFunds from '@/widgets/balance/addFunds.vue'
...
export default {
  ...
  computed: {
      ...
      this.getSP.forEach((sp) => {
        ...
        sp.locations.forEach((location) => {
          const plan = this.allPlans.find(({ type, uuid }) => {
            ...
            const { billing_plans: billingPlans } = sp.meta.showcase[this.showcase]
            const isTypesEqual = type === location.type
            const isPlanInclude = billingPlans?.includes(uuid)

            if (billingPlans.length < 1) return true
    ...
    productFullPriceOVH () {
      ...
      for (const range of this.getPlan.fee.ranges) {
    ...
    itemSP () {
      ...
      this.$store.dispatch('nocloud/plans/fetch', {
        ...
        .then(({ pool }) => {
          ...
          const [showcase] = keys.find(([key, value]) => {
            const plan = this.filteredPlans.find(({ uuid }) => value.billing_plans.includes(uuid))
      ...
      const { vars } = this.itemSP

      if (vars.min_drive_size) {
        this.options.disk.min = vars.min_drive_size.value[type]
      }
      if (vars.max_drive_size) {
        this.options.disk.max = vars.max_drive_size.value[type]
      }
    },
    'options.os.name' () {
      if (this.options.disk.min > 0) return
      const { id } = this.options.os
      const os = this.itemSP.publicData.templates[id]

      this.options.disk.min = os.min_size / 1024
    },
