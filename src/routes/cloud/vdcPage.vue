    ...
    <div class="container">
      <div class="service-page-card">
        <template v-if="service.uuid">
          ...
          <a-row :gutter="[10, 10]">
            <a-col v-for="item in info" :key="item.key" :md="12" :xs="12" :sm="12">
              <div class="service-page__info">
                <div class="service-page__info-title">
...
const info = computed(() => {
  const resources = Object.keys(service.value?.resources ?? {})
    .map((key) => ({ title: options[key].title, key, type: 'number' }))
...
function sendDelete () {
  Modal.confirm({
    ...
    content: () => h(
      'div',
      { style: 'color: red' },
      i18n.t('All data will be deleted!')
    ),
    onOk: async () => {
      return instancesStore.deleteInstance(service.value.uuid)
        .then(() => {
          openNotification('success', { message: 'Done!' })
