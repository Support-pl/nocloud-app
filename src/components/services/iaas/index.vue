        ...
        <div class="order__option">
          ...
          <template v-if="$route.query.product">
            ...
            <a-card
              v-if="
                fetchLoading ||
                (addons[getProducts.id] && addons[getProducts.id].length > 0)
              "
        ...
        <a-row type="flex" justify="space-around">
          <a-col style="font-size: 1.5rem">
            <transition name="textchange" mode="out-in">
              <template v-if="!fetchLoading">
                {{
                  (+getProducts.price[options.period] ||
                    getProducts.price.value) + addonsPrice.total
                }}
                {{ getProducts.price.currency }}
              </template>
...
export default {
  ...
  computed: {
    ...
    getProducts() {
      ...
      const findedProduct =
        this.products.find(({ id }) => id === +this.$route.query.product) ??
      ...
      if (/<\/?[a-z][\s\S]*>/i.test(product.description)) {
        ...
        if (product.paytype === "free") {
          ...
          product.price = product.price.find(
            ({ currency }) => currency === this.currency.id
          );
        }
      ...
      return this.options.addons.reduce(
        (prev, curr) => {
          const { prices = [], billingcycle } =
            this.addons[this.getProducts.id]?.find(({ id }) => id === curr) ??
            {};

          const price = prices.find((el) => el.currency === this.currency.id);
          const value =
            +price[this.options.period] !== -1
              ? +price[this.options.period]
              : 0;

          const result = {
            value:
              billingcycle !== "free" ? prev.value + (value || 0) : prev.value,
            onetime:
              billingcycle === "onetime"
                ? prev.onetime + +price.monthly
                : prev.onetime,
          };
  ...
  watch: {
    "options.size"() {
      this.periods = Object.entries(this.getProducts.price ?? {})
        .filter(([key, value]) => key.match(/ly/) && value > -1)
        .map((el) => el[0]);
  ...
  created() {
    this.$api
      .get(this.baseURL, { params: { run: "get_payment" } })
      .then((res) => {
  ...
  methods: {
    ...
    fetch() {
      ...
      this.fetchServices()
        .then((res) => {
          const { prod } = Object.values(res).find(
            ({ group_name: group }) => group === this.$route.query.service
          );
          ...
          this.periods = Object.entries(this.products[0].price[0])
            .filter(([key, value]) => key.match(/ly/) && value > -1)
            .map((el) => el[0]);
          this.options.period = this.periods[0];
        })
    ...
    orderClickHandler() {
      const info = {
        run: "add_product",
        billingcycle: ["free", "onetime"].includes(this.getProducts.paytype)
          ? "monthly"
          : this.options.period,
