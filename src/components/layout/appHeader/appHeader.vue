        ...
        <div class="header__right-side">
              ...
              >
                ...
                >
                  ...
                  <a-popover
                    ...
                    :open="isVisible"
                    @open-change="
                      isOpen ? (isVisible = true) : (isVisible = !isVisible)
                    "
                  >
                    <template #content>
                      <a-range-picker
          ...
          <transition name="header__item-anim">
            <div
              v-if="
                viewport < 576 &&
                headers[active]?.buttons.length > 0 &&
                isLogged
              "
              class="header__button"
...
export default {
  ...
  computed: {
    ...
    headerTitle() {
      ...
      if (this.headers[this.active]) {
        ...
        return this.capitalize(this.$t(this.$route.meta.headerTitle));
      } else return "";
    },
    viewport() {
      ...
      Object.assign(this, {
        checkedList: statuses.filter((status) => {
          const filters = JSON.parse(
            localStorage.getItem("supportFilters") ?? "[]"
          );
  ...
  created() {
    if (config.whmcsSiteUrl) {
      this.$api
        .get(this.baseURL, { params: { run: "get_currencies" } })
        .then((res) => {
