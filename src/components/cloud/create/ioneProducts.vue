<template>
    ...
    >
      ...
      >
        <a-slider
          ...
          @change="
            (i) => setOptions(`${slider.key}.size`, resources[slider.key][i])
          "
  ...
  <div v-else class="order__grid">
    ...
    >
      <h1>{{ item }}</h1>
      <div>{{ $t("cpu") }}: {{ getProduct(item)?.cpu ?? "?" }} vCPU</div>
      <div>{{ $t("ram") }}: {{ getProduct(item)?.ram / 1024 ?? "?" }} Gb</div>
    </div>
  </div>
</template>
...
watch(
  ...
  async () => {
    ...
    setOptions("cpu.size", product?.cpu);
  }
);
